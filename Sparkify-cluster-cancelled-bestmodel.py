...
def calc_metrics(data, model, test):
    ...
    df1.write.mode('overwrite').json('s3n://amosvoron-sparkify/gbt-cancelled-metrics-f22h.json')
    ...
    df2.write.mode('overwrite').json('s3n://amosvoron-sparkify/gbt-cancelled-feature-importances-f22h.json')
...
if __name__ == "__main__":
    ...
    spark = SparkSession \
        .builder \
        .appName("Sparkify-LogisticRegression") \
        .getOrCreate()
    ...
    f_SubmitDowngradePerHour = page_hour('Submit Downgrade')

    f_DowngradePerHour = page_hour('Downgrade')
    ...
    # Best features (22)
    data = labels.join(f_NonSongCount, 'userId', 'outer') \
        ...
        .join(f_LogoutPerHour, 'userId', 'outer') \
        .join(f_RollAdvertPerHour, 'userId', 'outer') \
        .join(f_ThumbsDownPerHour, 'userId', 'outer') \
        .join(f_RollAdvertPerDay, 'userId', 'outer') \
        .join(f_DowngradePerHour, 'userId', 'outer') \
        .join(f_HomePerHour, 'userId', 'outer') \
        .join(f_SettingsPerHour, 'userId', 'outer') \
        .join(f_SubmitDowngradePerHour, 'userId', 'outer') \
    ...
    classifier = GBTClassifier(maxIter=200, maxDepth=5)
